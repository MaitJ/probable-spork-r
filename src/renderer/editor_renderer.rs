...
use crate::{renderer::Renderer, texture::Texture, RendererResources, WgpuStructs};

use super::{MeshManager, TransformInstance};
...
impl EditorRenderer {
    pub async fn new(
        ...
        let renderer = egui_wgpu::Renderer::new(
            device,
            texture_format,
            Some(crate::Texture::DEPTH_FORMAT),
            1,
        );
        ...
        } in clipped_primitive
        {
            match primitive {
                Primitive::Callback(callback) => {
                    let cbfn =
                        if let Some(c) = callback.callback.downcast_ref::<GamePreviewCallback>() {
                            c
                        } else {
    ...
    fn update_meshes(&mut self, mesh_instances: Vec<MeshInstance>) {
        for mesh_instance in mesh_instances.iter() {
            match self
                .mesh_manager
                .get_meshes_mut()
                .get_mut(mesh_instance.mesh_index)
            {
                Some(mesh) => {
                    if let Err(e) = mesh.update_instance_data(
                        mesh_instance.mesh_instance_index,
                        TransformInstance::from(&mesh_instance.local_transform),
                    ) {
                ...
                None => {
                    warn!(
                        "Couldnt find mesh instance (mesh_index: {}, mesh_instance_index: {})",
                        mesh_instance.mesh_index, mesh_instance.mesh_instance_index
                    );
                }
            }
        }
