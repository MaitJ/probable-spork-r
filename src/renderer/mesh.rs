...
use crate::{
    shader::{Shader, BIND_GROUP_POSTFIX},
    texture::Texture,
    vertex::Vertex,
};
...
impl MeshRenderer for TexturedMesh {
    ...
    fn update_camera(&self, queue: &wgpu::Queue, camera_uniform_slice: &[CameraUniform]) {
        ...
        match camera_buffer {
            Some(buffer) => {
                queue.write_buffer(buffer, 0, bytemuck::cast_slice(camera_uniform_slice))
            }
    ...
    fn write_instance_data(&self, queue: &wgpu::Queue) {
        let instance_data: Vec<TransformInstanceRaw> = self
            .instances
            .iter()
            .map(TransformInstanceRaw::from)
            .collect();
        queue.write_buffer(
...
impl TexturedMesh {
    pub fn from(
        ...
        shader: Arc<Shader>,
        texture: Texture,
    ) -> Result<TexturedMesh, anyhow::Error> {
