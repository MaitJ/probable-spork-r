...
use crate::errors::{ErrorIdentificator, GeneralError};
...
impl Shader {
    pub fn get_uniform(&self, label: &'static str) -> Result<&ShaderUniform, GeneralError> {
        match self.uniforms.iter().find(|uniform| uniform.label == label) {
    ...
    pub fn get_uniform_buffer(&self, label: &str) -> Option<&wgpu::Buffer> {
        let uniform = self.uniforms.iter().find(|uniform| uniform.label == label);
...
impl ShaderBuilder {
    ...
    pub fn add_uniform<T>(mut self, device: &wgpu::Device, label: &'static str, data: T) -> Self
    where
        T: Pod + Zeroable + Copy + Clone + Debug,
    {
    ...
    ) -> Result<Shader, ShaderBuilderError> {
        let layouts_ref = self
            .uniforms
            .iter()
