...
use super::{MeshRenderer, Transform};
...
impl MeshInstance {
    pub fn apply_transforms_to_mesh_instance(
        scene: &Scene,
        entity_id: usize,
        mut mesh_instance: MeshInstance,
    ) -> MeshInstance {
        match scene
            .component_storage
            .get_entity_component::<Transform>(&Entity(entity_id as u32))
        {
            Some(transform) => {
