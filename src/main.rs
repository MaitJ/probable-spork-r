...
use std::sync::Arc;
use test_tree::{INDICES, VERTICES};
use texture::Texture;
...
impl App {
    ...
    fn create_mesh(&mut self) -> Option<TexturedMesh> {
        ...
        let diffuse_texture =
            Texture::from_bytes(texture_bytes, device, queue, "Tree texture").unwrap();

        info!(
            "Tree texture format: {:?}",
            diffuse_texture.texture.format()
        );
        let default_shader = self.get_shader_by_label("basic_shader.wgsl");

        match default_shader {
            Some(shader) => {
                let textured_mesh = TexturedMesh::from(
                    String::from("happy-tree"),
                    device,
                    VERTICES,
                    INDICES,
                    shader,
            ...
            None => {
                warn!(
                    "Couldn't find shader ({}) for {}",
                    "basic_shader.wgsl", "tree"
                );
                None
            }
        }
...
async fn start() {
...

    {
        ...
        let mut renderer = EditorRenderer::new(
            &app.wgpu_structs,
            &app.window,
            app.wgpu_structs.config.format,
            pixels_per_point,
        )
        .await;
