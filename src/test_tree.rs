...
use cgmath::{Matrix4, Zero, Rotation3, InnerSpace};
...
const TREE_DISPLACEMENT: cgmath::Vector3<f32> = cgmath::Vector3::new(NUM_OF_TREES as f32 * 0.5, 0.0, NUM_OF_TREES as f32 * 0.5);
...
impl Instance {
    pub fn to_raw(&self) -> InstanceRaw {
        InstanceRaw { model: (Matrix4::from_translation(self.position) * Matrix4::from(self.rotation)).into() }
    }
}
...
impl ExampleTree {
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        let instances = (0..NUM_OF_TREES).flat_map(|z| {
            (0..NUM_OF_TREES).map(move |x| {
                let position = cgmath::Vector3::new(x as f32, 0.0, z as f32) - TREE_DISPLACEMENT;
